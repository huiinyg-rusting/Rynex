...
pit::init(100);
    ...
    if let Err(e) = drivers::block_device::init_block_device() {
        serial::write_str("WARN: block device not found: ");
        serial::write_str(e);
        serial::write_str("\n");
    }

    if drivers::block_device::is_block_device_ready() {
        ...
        serial::write_str(">>> Block test skipped (no block device)\n");
    }
